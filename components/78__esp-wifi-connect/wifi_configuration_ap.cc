...
// =============================================================================
// BT A2DP 蓝牙音响功能（CONFIG_BT_A2DP_ENABLE 条件编译）
// =============================================================================
...
static esp_err_t bt_status_handler(httpd_req_t* req) {
    ...
    cJSON_AddStringToObject(root, "state", "bridge_waiting");
    cJSON_AddStringToObject(root, "message", "Waiting for ESP32 Bridge...");
    ...
    return ESP_OK;
#else
    BtA2dpSource& a2dp = BtA2dpSource::GetInstance();
    cJSON* root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "state", a2dp.IsConnected() ? "connected" : "idle");
    cJSON_AddStringToObject(root, "device", a2dp.GetConnectedDeviceName().c_str());
    const char* resp_str = cJSON_PrintUnformatted(root);
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp_str, strlen(resp_str));
    cJSON_free((void*)resp_str);
...
static esp_err_t bt_scan_handler(httpd_req_t* req) {
    ...
    cJSON_AddStringToObject(root, "error", "Bridge mode only");
    const char* resp_str = cJSON_PrintUnformatted(root);
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp_str, strlen(resp_str));
    cJSON_free((void*)resp_str);
    ...
    return ESP_OK;
#else
    BtA2dpSource::GetInstance().StartScan();
    cJSON* root = cJSON_CreateObject();
...
// /bt/results - 获取扫描结果
static esp_err_t bt_results_handler(httpd_req_t* req) {
#ifndef CONFIG_IDF_TARGET_ESP32
    cJSON* root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "state", "bridge_waiting");
...
// /bt/connect - 连接蓝牙设备
static esp_err_t bt_connect_handler(httpd_req_t* req) {
#ifndef CONFIG_IDF_TARGET_ESP32
    cJSON* root = cJSON_CreateObject();
    cJSON_AddBoolToObject(root, "success", false);
    cJSON_AddStringToObject(root, "error", "Bridge mode only");
    ...
    return ESP_OK;
#else
    BtA2dpSource::GetInstance().Connect("");
    cJSON* root = cJSON_CreateObject();
    cJSON_AddBoolToObject(root, "success", true);
    const char* resp_str = cJSON_PrintUnformatted(root);
    httpd_resp_set_type(req, "application/json");
...
static esp_err_t bt_disconnect_handler(httpd_req_t* req) {
    ...
    return ESP_OK;
#else
    BtA2dpSource::GetInstance().Disconnect();
    cJSON* root = cJSON_CreateObject();
    cJSON_AddBoolToObject(root, "success", true);
    const char* resp_str = cJSON_PrintUnformatted(root);
    httpd_resp_set_type(req, "application/json");
...
// /bt/forget - 忘记蓝牙设备
static esp_err_t bt_forget_handler(httpd_req_t* req) {
#ifndef CONFIG_IDF_TARGET_ESP32
    cJSON* root = cJSON_CreateObject();
    cJSON_AddBoolToObject(root, "success", false);
    cJSON_AddStringToObject(root, "error", "Bridge mode only");
    ...
    return ESP_OK;
#else
    BtA2dpSource::GetInstance().Forget();
    cJSON* root = cJSON_CreateObject();
    cJSON_AddBoolToObject(root, "success", true);
    const char* resp_str = cJSON_PrintUnformatted(root);
    httpd_resp_set_type(req, "application/json");
...
// 注册 BT A2DP 路由
static void register_bt_routes(httpd_handle_t server) {
    httpd_uri_t bt_uris[] = {
        {"/bt/status", HTTP_GET, bt_status_handler, NULL},
        {"/bt/scan", HTTP_POST, bt_scan_handler, NULL},
        {"/bt/results", HTTP_GET, bt_results_handler, NULL},
        {"/bt/connect", HTTP_POST, bt_connect_handler, NULL},
        {"/bt/disconnect", HTTP_POST, bt_disconnect_handler, NULL},
        {"/bt/forget", HTTP_POST, bt_forget_handler, NULL},
    };
    for (auto& uri : bt_uris) {
