...
void ElectronEmojiDisplay::SetupUI() {
    ...
    // Adjust status bar position for network configuration mode
    DisplayLockGuard lock(this);
    if (status_bar_) {
        // Change status bar position - adjust the y offset as needed
        // For example, move it down by 20 pixels
        lv_obj_align(status_bar_, LV_ALIGN_TOP_MID, 0, 40);
    }
...
void ElectronEmojiDisplay::SetupChatLabel() {
    // Create/recreate the chat label under the display lock
    {
        ...
        // Set chat message label position - adjust the y offset as needed
        // For example, move it down by 100 pixels from the top
        lv_obj_align(chat_message_label_, LV_ALIGN_TOP_MID, 0, 140);
    }
